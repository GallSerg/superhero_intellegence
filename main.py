...
def get_the_most_intelligent_hero(superhero_list):
    ...
    for name in superhero_list:
        if name in sph_dict:
            current_hero = get_superhero_params(sph_dict, name)
            if current_hero['intelligence'] > most_intelligent[1]:
                most_intelligent = (name, current_hero['intelligence'])
